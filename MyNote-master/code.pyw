...
class MainWindow(QMainWindow):
    def __init__(self): ## инициализация
        ...
        try: ## открыть .json с примечаниями
        ...
        except: # создание .json, если такого нет
    ...
    def new_note(self): ## создание новой заметки
    ...
    def selectItem(self, item_name): ## функция, чтобы получить заметку из списка
        if item_name != None:
    ...
    def note_changed(self): # если заголовок заметки изменится, заголовок окна тоже изменится
        if self.get_note_title().strip() != "":
            self.setWindowTitle("MyNotes - " + self.get_note_title().strip())
        else:
            self.setWindowTitle("MyNotes - untitled")
        self.note_unsaved()
        if self.get_note_title().strip() == "":
            self.saveButton.setDisabled(True)
            self.actionSaveNote.setDisabled(True)
    ...
    def redraw_list_menu(self): ## перерисовка списка один за другим
    ...
    def note_unsaved(self, status=True): ## отмечает, что заметка несохранена
        if status:
    ...
    def load_note(self, item, last_item): ## загрузить заметку из памяти
        if item != last_item and item != None:
            ...
            if item != None:
                ...
                self.setWindowTitle("MyNotes - " + current_note_name)
                self.note_unsaved(False)
    ...
    def delete_note(self): ## удалить заметку
        self.last = None
    ...
    def closeEvent(self, event: QtGui.QCloseEvent): ## если пользователь хочет закрыть окно, мы сохраняем заметку [триггер]
    ...
    def close_app(self): ## закрытие приложения из кода
        with open("userFiles/data.json", "w", encoding="utf8") as file:
            self.file = {
                "notes_dict": self.notes_dict,
                "last": self.last
...
class TableWidgetItem(QtWidgets.QTableWidgetItem): ## реорганизация сортировки
...
class AboutWindow(QWidget): ## окно можно вызвать из верхней панели или нажав CTRL+H
    ...
    def help_show(self):
        if self.isVisible():
            ...
            self.move( ## выравнивание по центру
...
class AboutQuit(QWidget): ## окно активируется при попытке закрыть приложение без сохранения
    def __init__(self, *args):
        super().__init__()
    ...
    def save_changes_window(self): ## если заметка не сохранена - попросить сохранить
        self.move(
    ...
    def save_changes(self):
        ...
        with open("userFiles/data.json", "w", encoding="utf8") as file: ## запись в файл
            mainWin.file = {
                "notes_dict": mainWin.notes_dict,
                "last": mainWin.last
            }
            json.dump(mainWin.file, file, indent=4)
        if self.isVisible(): ## приложение закрывается только в том случае, если оно запрашивает выход, иначе просто сохраняется без выхода
            mainWin.close_app()
